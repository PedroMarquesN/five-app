...
const handleFileChange = (event) => {
  localData.value.image = event.target.files[0];
};
...
const uploadPhotos = async () => {
  try {
    ...
    formData.append('image', localData.value.image);
    ...
    photos.value.push(responseData.data);
  } catch (error) {
    console.error(error);
  }
};
  ...
  <section class="upload-photos">
    ...
    <form @submit.prevent="uploadPhotos">
      <div class="custom-uploader">
        <input type="file" id="fileUpload" class="file-input" accept=".pdf,.doc,.docx,.jpg,.jpeg,.png"
        ...
        <div v-if="localData.image" class="upload-feedback text-positive">
          Arquivo selecionado: {{ localData.image.name }}
        </div>
      </div>
      ...
      <div class="form-group">
        ...
        <textarea id="description" v-model="description" placeholder="Digite a descrição"></textarea>
      </div>
...
.custom-uploader {
  ...
  background: linear-gradient(to bottom, #f0f4f8, #f6f8fb);
  border-radius: 12px;
  padding: 40px;
  width: 100%;
  max-width: 500px;
  ...
  border: 1px dashed rgba(255, 99, 71, 0.6);
  transition: all 0.3s ease;
}

.custom-uploader:hover {
  box-shadow: 0 8px 20px rgba(255, 99, 71, 0.2);
}
...
.upload-feedback {
  ...
  font-weight: 600;
  color: #a15d1c;
  background-color: rgba(199, 153, 53, 0.1);
  border: 1px solid #c56e3b;
  border-radius: 8px;
  padding: 10px 15px;
  text-align: center;
